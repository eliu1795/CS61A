def make_bank(balance):
    ...
    def bank(message, amount):
        ...
        if message == 'withdraw' and amount > balance:
        ...
        elif ('deposit'!= message) or ('withdraw' != message):
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, p):
        ...
        nonlocal balance, wrongpass
        
        if len(wrongpass) == 3:
            return 'Too many incorrect attempts. Attempts: ' + str(wrongpass)
        elif p != password:
            wrongpass.append(p)
...
def merge(incr_a, incr_b):
    ...
    "*** YOUR CODE HERE ***"
    while next_a != None and next_b != None: # three conditions, format from video's hints lol...
        if next_a < next_b:
            yield next_a
    ...
    while next_a != None: # stop and check.
        yield next_a
        next_a = next(iter_a, None)
    if next_b != None:
        yield next_b
