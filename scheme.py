...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError
        make_frame = Frame(self)
        while formals != nil and vals != nil:
            make_frame.define(formals.first, vals.first)
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    evaluated = scheme_eval(expressions.first, env, expressions.rest == nil)
    if expressions.rest == nil or is_false_primitive(evaluated):
        return evaluated 
    return do_and_form(expressions.rest, env)
    # END PROBLEM 12
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair):
        body, formal = target.first, target.rest 
        if scheme_symbolp(body):
            validate_formals(formal)
            env.define(body, MacroProcedure(formal, expressions.rest, env))
            return body
    raise SchemeError
    # END Problem 20
